    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        for alg in "${algs_list[@]}"; do # ----------------------------------- ALG
          ...
          exp_path_client="/root/chainpaxos/logs/read_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
          exp_path_server="/root/chainpaxos/logs/read_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
          ...
          for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
            ...
            server_p_ids=()
            for server_node in "${server_nodes[@]}"; do
              sshpass -p root ssh "root@$server_node" "cd /root/chainpaxos/server && java -Xmx${xmx} -Xms${xms} \
            ...
            sleep 8
            echo "Starting clients (+ extra) and waiting for them to finish"
            unset client_p_ids
            client_p_ids=()
            for node in "${client_nodes[@]}"; do
            ...
            sshpass -p root ssh "root@$node" "cd /root/chainpaxos/client && java -cp chain-client.jar \
                                  site.ycsb.Client -t -s -P config.properties \
            											-threads 15 -p fieldlength=$payload \
            											-p hosts=$servers_without_port -p n_frontends=1 \
            											-p read_type=strong \
            											-p readproportion=0 -p insertproportion=1 \
                      						-p updateproportion=0" 2>&1 | sed "s/^/[c-$node [extra]] /" &
            client_p_ids+=($!)
