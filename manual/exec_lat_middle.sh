    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        for alg in "${algs_list[@]}"; do # ----------------------------------- ALG
          ...
          exp_path_client="/root/chainpaxos/logs/latency/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
          exp_path_server="/root/chainpaxos/logs/latency/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
            ...
            for node in "${client_nodes[@]}"; do
              sshpass -p root ssh "root@$node" "cd /root/chainpaxos/client && java -cp chain-client.jar \
                      site.ycsb.Client -t -s -P config.properties \
											-threads $n_threads -target 3000 -p fieldlength=$payload \
											-p hosts=$middleServer -p n_frontends=1 \
		...
		sshpass -p root ssh "root@$server_node" "pkill -f chain*\.jar" &
            done
