  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      for server_node in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$server_node" "cp /root/chainpaxos/zkOriginal/conf/zoo_sample.cfg /root/chainpaxos/zkOriginal/conf/zoo.cfg"
        i=1
        for snode in "${server_nodes[@]}"; do
          sshpass -p root ssh "root@$server_node" "echo server.${i}=${snode}:2888:3888 >> /root/chainpaxos/zkOriginal/conf/zoo.cfg"
          i=$((i + 1))
        done
      done
      ...
      server_p_ids=()
      for server_node in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$server_node" "cd /root/chainpaxos/zkOriginal && \
      ...
      for server_node in "${server_nodes[@]}"; do
        #ssshpass -p root ssh "root@$server_node" "pkill java" &
	sshpass -p root ssh "root@$server_node" "pkill java" &
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="/root/chainpaxos/logs/zk/${exp_name}/client/${n_servers}/${reads_per}/${payload}/original/${run}"
        exp_path_server="/root/chainpaxos/logs/zk/${exp_name}/server/${n_servers}/${reads_per}/${payload}/original/${run}"
