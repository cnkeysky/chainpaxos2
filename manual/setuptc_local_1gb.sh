...
print_and_exec() {
  ...
  sshpass -p root ssh -n "root@$node" "$cmd" 2>&1 | sed "s/^/[$node] /"
}
...
for node in "${nodes[@]}"; do
  ...
  print_and_exec "${node}" "sudo tc qdisc add dev eth0 handle ffff: ingress"
  print_and_exec "${node}" "sudo tc filter add dev eth0 parent ffff: protocol ip u32 match u32 0 0 action mirred egress redirect dev ifb0"
  ...
  print_and_exec "${node}" "sudo tc qdisc del dev eth0 root; sudo tc qdisc add dev eth0 root handle 1: htb default 1"
