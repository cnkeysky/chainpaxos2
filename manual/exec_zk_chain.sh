  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      for snode in "${server_nodes[@]}"; do
        ...
        sshpass -p root ssh "root@$snode" "rm -rf /tmp/zookeeper; mkdir /tmp/zookeeper && echo ${i} > /tmp/zookeeper/myid"
        i=$((i + 1))
      done
      ...
      for server_node in "${server_nodes[@]}"; do
        ...
        echo strongRead=false >> $bibel_conf_loc"
      done
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="/root/chainpaxos/logs/zk/${exp_name}/client/${n_servers}/${reads_per}/${payload}/chain/${run}"
        exp_path_server="/root/chainpaxos/logs/zk/${exp_name}/server/${n_servers}/${reads_per}/${payload}/chain/${run}"
