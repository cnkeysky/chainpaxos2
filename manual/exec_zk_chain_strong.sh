...
for run in $(# ------------------------------------------- RUN
  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
    ...
    for n_servers in "${n_servers_list[@]}"; do # --------------------------- N_SERVERS
      ...
      zoo_cfg_loc=/root/chainpaxos/apache-zookeeper-3.7.0-bin/conf/zoo.cfg

      i=1
      for snode in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$snode" "cp /root/chainpaxos/apache-zookeeper-3.7.0-bin/conf/zoo_sample.cfg $zoo_cfg_loc &&\
        rm -rf /tmp/zookeeper && mkdir /tmp/zookeeper && echo ${i} > /tmp/zookeeper/myid"
        i=$((i + 1))
      done

      bibel_conf_loc=/root/chainpaxos/apache-zookeeper-3.7.0-bin/conf/babel.conf
      for server_node in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$server_node" "cp /root/chainpaxos/apache-zookeeper-3.7.0-bin/conf/config.properties $bibel_conf_loc"
        sshpass -p root ssh "root@$server_node" "echo '' >> $bibel_conf_loc && echo initial_membership=${servers_without_port} >> $bibel_conf_loc && \
        echo quorum_size=${quorum_size} >> $bibel_conf_loc && echo initial_state=ACTIVE >> $bibel_conf_loc &&\
        echo strongRead=true >> $bibel_conf_loc"
      done

      echo -e "$BLUE Starting servers and sleeping 10 for LOADING $NC"
      unset server_p_ids
      server_p_ids=()
      for server_node in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$server_node" "cd /root/chainpaxos/apache-zookeeper-3.7.0-bin && \
            bin/zkServer.sh start ./conf/zoo.cfg" 2>&1 | sed "s/^/[s-$server_node] /" &
        server_p_ids+=($!)
        sleep 0.2
      done
      sleep 10
      echo -e "$BLUE Loading data $NC"
      sshpass -p root ssh "root@${client_nodes[0]}" "cd /root/chainpaxos/client && java -cp chain-client.jar:. \
                          site.ycsb.Client -t -s -P config.properties -load \
                          -p zookeeper.syncRead=false \
      ...
      for server_node in "${server_nodes[@]}"; do
	      sshpass -p root ssh "root@$server_node" "pkill java" &
      done
      ...
      backup_pids=()
      for server_node in "${server_nodes[@]}"; do
        sshpass -p root ssh "root@$server_node" "rm -rf /tmp/zookeeper_bak && mkdir /tmp/zookeeper_bak && cp -r /tmp/zookeeper/* /tmp/zookeeper_bak/" &
        backup_pids+=($!)
      done
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="/root/chainpaxos/logs/zk_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/chain/${run}"
        exp_path_server="/root/chainpaxos/logs/zk_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/chain/${run}"

        for server_node in "${server_nodes[@]}"; do
            sshpass -p root ssh "root@$server_node" "mkdir -p ${exp_path_server}"
        done

        for node in "${client_nodes[@]}"; do
            sshpass -p root ssh "root@$node" "mkdir -p ${exp_path_client}"
        done
        ...
        for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
          ...
          for server_node in "${server_nodes[@]}"; do
              sshpass -p root ssh "root@$server_node" "rm -rf ${exp_path_server}/${n_threads}_*"
          done

          for node in "${client_nodes[@]}"; do
              sshpass -p root ssh "root@$node" "rm -rf ${exp_path_client}/${n_threads}_*"
          done
          ...
          for server_node in "${server_nodes[@]}"; do
            sshpass -p root ssh "root@$server_node" "rm -rf /tmp/zookeeper/* && cp -r /tmp/zookeeper_bak/* /tmp/zookeeper/" &
            backup_pids+=($!)
          done
          for pid in "${backup_pids[@]}"; do
            wait "$pid"
          ...
          echo -e "$BLUE Starting servers and sleeping 10 $NC"
          ...
          sleep 10
          echo "Starting clients (+ extra writer) and waiting for them to finish"
          unset client_p_ids
          client_p_ids=()
          i=0
          for node in "${client_nodes[@]}"; do
            sshpass -p root ssh "root@$node" "cd /root/chainpaxos/client && java -cp chain-client.jar:. \
          ...
          sshpass -p root ssh "root@$node" "cd /root/chainpaxos/client && java -cp chain-client.jar:. \
                                site.ycsb.Client -t -s -P config.properties \
                                -p db=ZKClient -p hosts=$servers_without_port \
          											-threads 10 -p fieldlength=$payload \
          											-p zookeeper.syncRead=false \
          											-p readproportion=0 -p insertproportion=0 \
          											-p updateproportion=1" 2>&1 | sed "s/^/[c-$node [extra]] /" &
          client_p_ids+=($!)
