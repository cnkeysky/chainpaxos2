    ...
    for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
      ...
      for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
        ...
        for alg in "${algs_list[@]}"; do # ----------------------------------- ALG
          ...
          exp_path_client="/root/chainpaxos/logs/cpu_threads/${exp_name}/client/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
          exp_path_server="/root/chainpaxos/logs/cpu_threads/${exp_name}/server/${n_servers}/${reads_per}/${payload}/${alg}/${run}"
          ...
          for n_threads in "${threads_list[@]}"; do # -------------------- N_THREADS
            ...
            for server_node in "${server_nodes[@]}"; do
              #sshpass -p root ssh "$server_node" "pkill java" &
