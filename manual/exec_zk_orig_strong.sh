  ...
  for payload in "${payloads_list[@]}"; do # ------------------------- PAYLOADS
	...
	sshpass -p root ssh "root@$server_node" "pkill java" &
      ...
      for reads_per in "${reads_list[@]}"; do # ---------------------------  READS_PER
        ...
        exp_path_client="/root/chainpaxos/logs/zk_strong/${exp_name}/client/${n_servers}/${reads_per}/${payload}/original/${run}"
        exp_path_server="/root/chainpaxos/logs/zk_strong/${exp_name}/server/${n_servers}/${reads_per}/${payload}/original/${run}"
